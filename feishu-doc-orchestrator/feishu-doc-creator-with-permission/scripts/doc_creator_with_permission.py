...
def create_document(token, config, title):
    """创建飞书云盘文档 - 使用 docx API 在云盘文件夹中创建"""
    # 获取云盘 folder_token（默认使用 DYPXf8ZktlOCIXdmGq3cfjevn2F）
    folder_token = config.get('FEISHU_DRIVE_FOLDER_TOKEN', 'DYPXf8ZktlOCIXdmGq3cfjevn2F')
    
    # 使用 docx API 创建文档，指定 folder_token 保存到云盘
    ...
    if result.get("code") == 0:
        ...
        raise Exception(f"创建云盘文档失败: {result}")
